...
/// <summary>
/// Prompts that merge ordered messenger-panel OCR strings into one cleaned chat transcript.
/// </summary>
public static class MessengerSplitMergePrompts
{
    static readonly LocalizedTextSet _prompts = new()
    {
        [nameof(MergeInstructions)] = new LocalizedText
        {
            [OcrLanguage.English] =
                """
        ...
        ["PanelBlock"] = new LocalizedText
        {
            [OcrLanguage.English] = "------ Panel {panelNumber}:\n{text}",
            [OcrLanguage.Russian] = "------ Панель {panelNumber}:\n{text}",
        },
    };

    public static string MergeInstructions(
        IReadOnlyList<OcrLanguage>? languages,
        IReadOnlyList<string> panelTexts)
    {
        var sb = new StringBuilder();
        sb.AppendLine(_prompts.Get(nameof(MergeInstructions), languages).Trim());
        sb.AppendLine();

        var panelBlockPrompt = _prompts.Get("PanelBlock", languages);

        for (var i = 0; i < panelTexts.Count; i++)
        {
            sb.AppendLine(
                panelBlockPrompt
                    .Replace("{panelNumber}", (i + 1).ToString())
                    .Replace("{text}", panelTexts[i] ?? string.Empty));
